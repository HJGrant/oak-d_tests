...
def compute_depth(intrinsics: o3d.core.Tensor, disparity_in_pixels: np.ndarray, baseline_cm: float):
    # Convert baseline from centimeters to meters
    baseline_meters = baseline_cm * 10

    # Avoid division by zero in disparity
    disparity_in_pixels[disparity_in_pixels == 0] = 1

    # Extract fx from the intrinsics tensor
    fx = intrinsics[0, 0].item()  # Convert to scalar for computation

    print("fx: " + str(fx))
    print("Baseline: " + str(baseline_meters) + " meters")
    print("Input Disparity Dtype: " + str(disparity_in_pixels.dtype))

    # Compute depth: Z = (fx * baseline) / disparity
    depth = (fx * baseline_meters) / disparity_in_pixels

    # Print some information about the depth range for validation
    print(f"Depth range: min={np.min(depth)}, max={np.max(depth)}")

    # Return depth as a float32 (in meters if calibration is correct)
...
class ReconstructionWindow:
    ...
    def update_main(self):
        ...
        pipeline, depth, camRGB = setup_oak_d()

        with dai.Device(pipeline) as device:
            rgbQueue = device.getOutputQueue(name="rgb", maxSize=4, blocking=False)
            dispQueue = device.getOutputQueue(name="disp", maxSize=4, blocking=False)
            intrinsic, _, baseline = get_oak_d_calib(device)

            #depth_file_names, color_file_names = load_rgbd_file_names(self.config)
            #intrinsic = load_intrinsic(self.config)
            
            for i in range(10):
                frameRgb_uint8 = rgbQueue.get().getCvFrame() #read in as uint8
                frameDisp_uint8 = dispQueue.get().getFrame() #read in as uint8

                frameRgb_float32 = frameRgb_uint8.astype(np.float32) / 255.0
                frameDisp_float32 = frameDisp_uint8.astype(np.float32) / 255.0

                #frameDisp = (frameDisp * (255 / depth.initialConfig.getMaxDisparity())).astype(np.uint8)

                cv2.imshow("Disparity", frameDisp_float32)
                cv2.imshow("Color", frameRgb_float32)

                frameDepth_float32 = compute_depth(intrinsic, frameDisp_uint8, baseline)

                #print(frameDepth.dtype)
                cv2.imshow("Depth", frameDepth_float32)
            ...
            depth_ref = o3d.t.geometry.Image(frameDepth_float32)
            color_ref = o3d.t.geometry.Image(frameRgb_float32)
            ...
            while not self.is_done:
                ...
                frameRgb_uint8 = rgbQueue.get().getCvFrame()
                frameDisp_uint8 = dispQueue.get().getFrame()
                ...
                depth = o3d.t.geometry.Image(frameDepth_float32)
                color = o3d.t.geometry.Image(frameRgb_float32)
